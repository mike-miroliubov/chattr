...
import config.ExchangeShardRegion
...
object GroupExchange {
  ...
  def apply(groupId: String, persistenceId: PersistenceId)
           (using exchangeShardRegion: ExchangeShardRegion): Behavior[OutgoingMessage | Command] = {
    def handleEvent(state: Option[State], event: Event) = {
      state match {
        case Some(s) =>
          event match {
    ...
    def handleCommand(context: ActorContext[OutgoingMessage | Command],
                      state: Option[State],
